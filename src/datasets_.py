...
class CIRCODataset(Dataset):
    ...
    def read_from_annotation(self, annotation, key):
        ans = ''
        # print(annotation.keys())
        if key in annotation:
            ans = annotation[key]
        if isinstance(ans, list):
            return ans[:args.nums_caption]
        else:
            return ans
    ...
    def __getitem__(self, index) -> dict:
        ...
        if self.mode == 'relative':
            ...
            blip2_caption_opt = self.read_from_annotation(self.annotations[index], 'blip2_caption')
            gpt_caption = self.read_from_annotation(self.annotations[index], 'gpt-3.5-turbo')
            gpt_caption_t5 = self.read_from_annotation(self.annotations[index], 'gpt-3.5-turbo_t5')
            gpt_caption_none = self.read_from_annotation(self.annotations[index], 'gpt-3.5-turbo_none')
            ...
            multi_gpt4_opt = self.read_from_annotation(self.annotations[index], 'multi_gpt-4_opt')
