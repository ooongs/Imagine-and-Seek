...
import cv2
import math
from .migc_arch import MIGC, NaiveFuser
...
class StableDiffusionMIGCPipeline(StableDiffusionPipeline):
    ...
    def __call__(
        ...
        if latents==None:
            ...
            latents = latents
        init_latents = latents.clone().detach()
        ...
        if not return_dict:
            return (image, None), init_latents

        if return_inital_latents:
